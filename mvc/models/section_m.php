...
class Section_m extends MY_Model {
	...
	function get_allsection($id) {
		$query = $this->db->get_where('section', array('facultiesID' => $id));
		return $query->result();
	}
	...
	function get_join_section($id) {
		...
		$this->db->where('section.facultiesID', $id);
		$query = $this->db->get();
		return $query->result();
	}

	function get_join_section_with_faculties() {
		$this->db->select('*');
		$this->db->from('section');
		$this->db->join('faculties', 'section.sectionID = faculties.facultiesID', 'LEFT');
		// $this->db->where('section.facultiesID', $id);
		$query = $this->db->get();
		return $query->result();
	}
